...
#let us try
class ATM:
    def __init__(self, balance = 0, interest_rate = 0.1):#balance always starts 1, interest rate 0.1
    ...
    def deposit(self, account):
        #we want the account value to increase
        self.balance += account
    #check_withdrawal is the third function that is not doing the same thing upon initialization
    def check_withdrawal(self, account):
        #we want the balance value to decrease
        self.balance -= account
    #calc_interest is the fourth function that is not doing the same thing upon initialization
