...
class Reader_cbl:
    ...
    def save_plot_a_log(self, for_that_well, dpi_user = 300):
        ...
        plt.show()
        print('Your visualization of the well logs have been saved as well_report_plot.jpeg')
    ...
    def save_plot_a_log_with_hlines(self, for_that_well, top_line, bottom_line, x_min_vdl, x_max_vdl, x_min_amp, x_max_amp, dpi_user = 300):
        ...
        plt.savefig('well_report_plot.jpeg', dpi = dpi_user, bbox_inches = "tight")#added bbox_inches to stop the save fig from cutting off text
        plt.show()
        print('Your annotation of the well logs have been saved as well_report_plot.jpeg')
    
#following fpdf documentation, I will create a separate class for the document generation http://www.fpdf.org/
class PDF(FPDF):
    def lines(self):
        self.rect(.2, .2, 8.0,10.5)#this the rectangle outside
    def header(self):
        # # Plot
        # self.image('well_report_plot.png', 10, 8, 33)
        self.set_font('Arial', 'B', 18)# Arial bold 18        
        # self.cell(80)# Move to the right        
        self.cell(w=7.7, h=1.5, align='C', txt=f"Cement Bond Log Report for {well}", border=0)# Title. The w adjusts left and right and it seems messed up.
